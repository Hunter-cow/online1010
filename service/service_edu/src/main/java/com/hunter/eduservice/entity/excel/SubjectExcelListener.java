package com.hunter.eduservice.entity.excel;
...
public class SubjectExcelListener extends AnalysisEventListener<SubjectData> {
    ...
    //因为SubjectExcelListener不能交给spring进行ioc管理，需要自己手动new，所以不能注入其他对象（autowired）
    //所以采用构造函数来注入edusubjectservice来实现对数据库的写操作
    //不能实现数据库操作
    ...
    public void invoke(SubjectData subjectData, AnalysisContext analysisContext) {
        ...
        if (subjectData == null) {
            throw new ConsumeException(20001, "添加失败");
        }
